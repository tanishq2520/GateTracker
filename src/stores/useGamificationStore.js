...
const getGamificationUid = () => useAuthStore.getState().uid;
...
export const useGamificationStore = create((set, get) => ({
  ...
  resetGamification: () => set(resetState()),

  loadGamification: async (uidArg) => {
    const uid = uidArg || getGamificationUid();
    if (!uid) {
    ...
    get().initWeeklyChallenge(uid);
  },

  awardXP: (amount, label) => {
    const uid = getGamificationUid();
    const { totalXP } = get();
    ...
    saveStats(uid, { totalXP: newXP, level: newLvl.level, lastUpdated: todayISO() });
    const event = { date: todayISO(), label, amount, ts: Date.now() };
    appendXPEvent(uid, event);
    set((state) => ({ xpLog: [event, ...state.xpLog].slice(0, 20) }));

    if (newLvl.level === 10) {
      get().awardBadge('gate_ready');
    }
  },
  ...
  processStreak: async () => {
    ...
    const newFreezes = streak.lastFreezeReset && streak.lastFreezeReset >= thisMonthFirst
    ...
    await saveStreak(uid, newStreakData);

    const shown = milestones.shown || [];
    const unshown = STREAK_MILESTONES.filter((milestone) => newCurrent >= milestone && !shown.includes(milestone));
    if (unshown.length > 0) {
      const updated = [...shown, ...unshown];
      set({
        milestones: { shown: updated },
        streakMilestoneBanner: unshown[unshown.length - 1],
      });
      saveMilestones(uid, { shown: updated });
    }
  ...
  checkAndAwardBadges: (ctx) => {
    const { earnedBadges } = get();
    const has = (id) => earnedBadges.some((badge) => badge.id === id);
    const { subjects = {}, tests = {}, logs = {}, hour, subjectId } = ctx;
    const completed = Object.values(subjects).filter((subject) => subject.status === 'completed');
    ...
    if (!has('speed_runner') && subjectId && subjects[subjectId]) {
      ...
      if (subject.status === 'completed' && subject.completedOn && subject.plannedEndDate) {
        const diff = Math.round((new Date(subject.plannedEndDate) - new Date(subject.completedOn)) / 86400000);
        if (diff >= 3) {
          get().awardBadge('speed_runner');
        }
      }
    }
    ...
    if (!has('sniper') && Object.values(tests).some((test) => test.totalMarks > 0 && test.scored / test.totalMarks >= 0.9)) {
    ...
    if (!has('improving')) {
      ...
      if (sorted.length >= 3) {
        ...
        for (let i = 1; i < sorted.length; i += 1) {
          const previous = sorted[i - 1].totalMarks > 0 ? sorted[i - 1].scored / sorted[i - 1].totalMarks : 0;
          const current = sorted[i].totalMarks > 0 ? sorted[i].scored / sorted[i].totalMarks : 0;
          if (current > previous) {
            cons += 1;
            if (cons >= 2) {
              get().awardBadge('improving');
              break;
            }
          } else {
            cons = 0;
  ...
  initWeeklyChallenge: async (uidArg) => {
    const uid = uidArg || getGamificationUid();
    if (!uid) return;

    const { weeklyChallenge } = get();
    const weekId = getWeekId();
    if (weeklyChallenge && weeklyChallenge.weekId === weekId) return;
    ...
    await saveWeeklyChallenge(uid, next);
  },
  ...
  updateChallengeProgress: async (actionType) => {
    ...
    if (weeklyChallenge.weekId !== getWeekId()) {
      get().initWeeklyChallenge(uid);
      return;
    }
    ...
    await saveWeeklyChallenge(uid, updated);

    if (completed) {
      get().awardXP(150, 'Weekly challenge completed');
    }
  },
